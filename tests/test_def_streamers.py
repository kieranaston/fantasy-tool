...
def test_skill_board_keeps_one_player_per_team() -> None:
    from src.def_streamers.rb_board import build_rb_board
    ...
    # Already sorted by avg desc (loader order); only Walker should remain for SEA.
    player_avgs = [
        {
            "player_id": "1",
            "player_name": "Kenneth Walker III",
    ...
    teams = [p["team"] for p in board["players"]]
...
def test_build_qb_board_implied_and_rush() -> None:
    ...
    assert "rolling 8-game" in board["stats_note"]
    row = board["players"][0]
