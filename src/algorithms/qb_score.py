...
from src.config.scoring import QB_WEIGHTS
...
def build_qb_rankings(
    ...
    *,
    ...
    """Build QB rankings from per-game opportunity and efficiency rates."""
    ...
    frame = (
        ...
        .filter((pl.col("games_played") > 0) & (pl.col("pass_attempts") > 0))
        .with_columns(
            (pl.col("pass_attempts") / pl.col("games_played")).alias("pass_attempts_pg"),
            (pl.col("rush_attempts") / pl.col("games_played")).alias("rush_attempts_pg"),
            (pl.col("passing_yards") / pl.col("pass_attempts")).alias("yards_per_attempt"),
        )
    )
    ...
    pass_n = normalize_values([float(r["pass_attempts_pg"]) for r in raw])
    rush_n = normalize_values([float(r["rush_attempts_pg"]) for r in raw])
    ypa_n = normalize_values([float(r["yards_per_attempt"]) for r in raw])

    rows: list[dict] = []
    for idx, player in enumerate(raw):
        components = {
            "pass_attempts_pg": round(pass_n[idx], 1),
            "rush_attempts_pg": round(rush_n[idx], 1),
        ...
        rows.append({
            ...
            "metrics": {
                "pass_attempts_pg": round(float(player["pass_attempts_pg"]), 1),
                "rush_attempts_pg": round(float(player["rush_attempts_pg"]), 1),
